...
function migrateUsers() {
  db.prepare(`UPDATE users SET role = 'user' WHERE email != ?`).run('admin@site.local');
  db.prepare(`UPDATE users SET role = 'admin' WHERE email = ?`).run('admin@site.local');
}

function seedFootballPosts() {
...
function upgradeLegacyWelcomePost() {
  const n = db.prepare('SELECT COUNT(*) AS c FROM posts').get().c;
  if (n !== 1) return;
  const p = db.prepare('SELECT slug FROM posts LIMIT 1').get();
